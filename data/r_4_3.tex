...
\begin{important}
    \textbf{LF Mapping Property:} A fundamental property of the BWT which states that the $i$-th occurrence of a specific character in the First column (F) of the Burrows-Wheeler matrix corresponds to the exact same original character in the sequence as the $i$-th occurrence of that character in the Last column (L).
\end{important}
...
\begin{itemize}
    ...
    \item \textbf{B-ranking:} A cumulative count of how many times a character has been observed so far in a specific column (either F or L) starting from the top.
\end{itemize}
...
\begin{enumerate}
    \item Start at the row containing the unique end character (e.g., `\$`) in the F column. Because `\$` is lexicographically smallest, it is always in the first row.
    \item Look at the corresponding character in the L column of that row. This character (e.g., $A_0$) is the last character of the original sequence (immediately preceding `\$`).
    \item Find the matching character with the same B-rank (e.g., $A_0$) in the F column.
    \item Look at the L column in this new row. This character immediately precedes $A_0$ in the original text.
    \item Repeat this jump from L to F until you return to the `\$` character.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{B-rank Storage:} Explicitly storing the B-rank for every character in the 3 billion letter human genome would require billions of integers, defeating the memory compression benefits of the BWT.
    \item \textbf{Position Lookup:} Once a match is found, the BWT matrix does not tell us \textit{where} in the original genome the match occurred. The standard Suffix Array provides this, but it requires 12+ GB of memory.
\end{enumerate}

\section{The FM Index: Optimizing the BWT for Read Mapping}

To solve the issues of the naive BWT search, the \textbf{FM Index} (Full-text index in Minute space) augments the BWT with specialized auxiliary data structures. This allows tools like BWA, Bowtie, and SOAP2 to perform incredibly fast alignments.

\subsection{The Tally Table (Occ Table) with Checkpoints}
To solve both the search efficiency and B-rank storage issues, we define an $Occ$ (occurrences) table, also known as a tally table. This table tracks the cumulative count of each character encountered in the L column up to a given row.
...
\begin{important}
    \textbf{Tally Table Checkpointing:} To save memory, we do not store every row of the tally table. We only save "checkpoints" every $k$ rows (e.g., $k=128$). To find the exact count at an unstored row, we jump to the nearest checkpoint and manually count the few characters in the L column between the checkpoint and our target row.
\end{important}
...
\begin{enumerate}
    ...
    \item Once we hit a row that \textit{does} have a stored positional value (e.g., position 500), we add the number of hops we made (e.g., 2) to find the original position of our match (502).
\end{enumerate}
With $k=128$, we make a maximum of 127 hops, making position lookups extremely fast and bounding the selective suffix array to a fraction of the memory. The total FM index size for the human genome is roughly 1.5 GB.

\section{BWA: Formalizing Exact Matching}

The Burrows-Wheeler Aligner (BWA) formalizes the backward search approach using interval notation. Every substring $W$ corresponds to a continuous \textbf{Suffix Array Interval} denoted as $[\underline{R}(W), \overline{R}(W)]$, where $\underline{R}$ is the lower bound and $\overline{R}$ is the upper bound.

\begin{important}
    \textbf{BWA Precalculations:}
    \begin{itemize}
        \item $C(a)$: the number of characters in the original text $T$ that are lexicographically strictly smaller than character $a$. This is exactly the Cumulative Index count for the F column.
...
The search iteratively computes the interval for a pattern $W$ of length $m$. Let $W_k = W[k \dots m-1]$ be a suffix of $W$, where $W_k = a_k W_{k+1}$ (meaning $a_k$ is the current character being prepended). The interval updates from $W_{k+1}$ to $W_k$ as follows:
...
\begin{enumerate}
    ...
    \item Interval for "go" is $[1, 2]$.
\end{enumerate}

\section{BWA: Inexact Matching}

Practical read aligners must accommodate sequencing errors and biological mutations (e.g., SNPs, where $\sim 1$ in $1,000$ bases differ from the reference).
...
Conceptually, inexact matching backwards maps cleanly to a \textbf{Prefix Trie}. A prefix trie is identical to a suffix trie of the reversed text. A path from the root to a leaf spells the reverse of a prefix.

To search inexactly, we execute a Depth-First Search (DFS) starting from the root. Every time we encounter a character in the trie that does not match our target search pattern, we increment an error counter. If the error counter exceeds $d_{max}$, we prune that branch and backtrack. Note that building a real prefix trie for a genome causes memory explosion, so this DFS traversal is simulated virtually using the BWT intervals.

\subsection{Early Pruning Heuristic: CalculateD}

To prevent the DFS from exploring hopeless branches deep in the recursion tree, BWA employs a heuristic function called \texttt{CalculateD(W)}. It precalculates the \textit{minimum} number of mismatches required for any prefix of the search string.
...
This algorithm loops linearly over the search string $W$. It checks increasingly long substrings. The moment a substring $W[j..i]$ is not found in the original text $T$, it guarantees at least one error exists in that block. It increments the error counter $z$, updates the lower-bound array $D$, and resets the substring search boundary $j$ to the next character.

\textbf{Example:} Early Pruning with `CalculateD`.
Searching for $W = \text{"LOL"}$ inside $T = \text{"GOOGOL"}$ with $d_{max} = 1$.
\begin{enumerate}
    \item $D(0) = 0$ because "L" exists in $T$.
    \item $D(1) = 1$ because "LO" does not exist in $T$.
...
With `CalculateD` computed, the actual BWT inexact search uses a recursive function \texttt{InexRecur}.
...
\begin{itemize}
    ...
    \item The maximum remaining allowed mismatches $z$.
...
This recursive function formalizes the exact same Depth-First Search approach discussed on the prefix trie, but operates entirely within the bounded memory of the BWT matrix representation.
