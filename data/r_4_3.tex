...
\mysidefigure{$\bullet$ \textbf{T-ranking:} Numbering characters based on their sequential appearance in the original unrotated string (e.g., $A_0, A_1, A_2$). Storing this is highly inefficient for a genome.}{\myimage{4_3/2.png}\\}[0.7]
\mysidefigure{$\bullet$ \textbf{B-ranking:} A cumulative count (or zero-based index) of how many times a character has been observed so far in a specific column (either F or L) starting from the top down to the current position. It replaces the memory-intensive T-ranking.}{\myimage{4_3/3.png}\\}[0.7]
...
\mysidefigure{\textbf{Example:} Reconstructing an original string from the BWT.
...
\end{enumerate}}{\myimage{4_3/4.png}\\}[0.8]
...
\mysidefigure{If we know we have 1 `\$`, 5 `A`s, and 2 `B`s, we can deduce mathematically that the `C`s must start at index $1 + 5 + 2 = 8$ in the F column. This compacts the entire F column down to an array of just 4 integers (for DNA).}{\myimage{4_3/5.jpg}\\}[0.55]
...
\mysidefigure{\myimage{4_3/6.png}\\}{\myimage{4_3/7.png}\\}[0.5]
...
\myimage{4_3/8.jpg}{}[width=0.5\textwidth]
...
\myimage{4_3/9.png}{}[width=0.5\textwidth]
...
\mysidefigure{\textbf{Example:} Exact matching algorithm applied iteratively.
...
\mysidefigure{\myimage{4_3/1.jpg}\\}{Diagram showing a prefix trie for the string GOOGOL. A depth-first search (DFS) path is highlighted for the search word "LOL" with $d_{max}=1$, showing how branches are pruned or explored based on the mismatch threshold.}[0.6]
