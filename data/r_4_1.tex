...
%\myimage{4_1/1.jpg}{Alignments in the IGV (Integrative Genomics Viewer) Browser. The image shows a target reference sequence at the bottom, an amino acid translation track, and a stack of aligned sequencing reads (gray horizontal bars) on top. Colored marks within the reads indicate mismatches or potential mutations.}[width=0.5\textwidth]

%\myimage{4_1/7.jpg}{}

\mysidefigure{\myimage{4_1/1.jpg}\\}{\myimage{4_1/7.jpg}\\}[0.33]
Alignments in the IGV (Integrative Genomics Viewer) Browser. The image shows a target reference sequence at the bottom, an amino acid translation track, and a stack of aligned sequencing reads (gray horizontal bars) on top. Colored marks within the reads indicate mismatches or potential mutations.
...
%\myimage{4_1/2.png}{Naive string search illustration. The string "nas" is aligned under "bananasavannah", shifting one position to the right after every mismatch until a perfect match is found.}[width=0.2\textwidth]

\mysidefigure{\myimage{4_1/2.png}\\}{Naive string search illustration. The string "nas" is aligned under "bananasavannah", shifting one position to the right after every mismatch until a perfect match is found.}[0.4]
...
\begin{itemize}
    \item Each edge is labeled with \textbf{exactly} one character \(c \in \Sigma\).
...
%\myimage{4_1/3.jpg}{A trie data structure built from the words "banana", "bandana", "nasa", "anna", and "annex". The root node branches to 'b', 'n', and 'a'. Each path leads uniquely to a termination character '\$'.}[width=0.5\textwidth]

\mysidefigure{\myimage{4_1/3.jpg}\\}{A trie data structure built from the words "banana", "bandana", "nasa", "anna", and "annex". The root node branches to 'b', 'n', and 'a'. Each path leads uniquely to a termination character '\$'.}[0.5]
...
\myimage{4_1/8.jpg}{Suffix trie of the string T="MISSISSIPPI\$". The tree branches out from the root showing every possible suffix path. Deeply branching nodes represent repeated substrings, like "SSI" and "I".}[width=0.7\textwidth]
...
% [IMAGE: Transformation from a Suffix Trie to a Suffix Tree for the string "MISSISSIPPI$". Long single-path chains like "ISSIPPI$" are merged into a 
\myimage{4_1/9.jpg}{Transformation from a Suffix Trie to a Suffix Tree for the string "MISSISSIPPI\$". Long single-path chains like "ISSIPPI\$" are merged into a}[width=0.5\textwidth]
...
single multi-character edge label, vastly reducing the node count.]
\myimage{4_1/6.jpg}{Compression and usnig a pointer to the original string} 
...
\begin{table}[ht]
    ...
    \begin{tabular}{r | l || r | l}
        \toprule
        \textbf{Index} & \textbf{Original Suffix} & \textbf{SA Index} & \textbf{Sorted Suffix} \\
        \midrule
        0  & MISSISSIPPI\$ & 11 & \$ \\
        1  & ISSISSIPPI\$  & 10 & I\$ \\
        2  & SSISSIPPI\$   & 7  & IPPI\$ \\
        ...
        \bottomrule
    \end{tabular}
    \caption*{Original and lexicographically sorted suffixes for MISSISSIPPI\$}
\end{table}
...
\myimage{4_1/10.jpg}{}[width=0.7\textwidth]
